                ...
                if i != 'axisNames':
                    ...
                    for group in hdf_file[f'/WaterLevel/{i}/']:
                        if group != 'Positioning':
                            ...
                            number_of_times = int(
                                hdf_file[f'/WaterLevel/{i}/{group}/'].attrs['numberOfTimes'])
                            interval = int(
                                hdf_file[f'/WaterLevel/{i}/{group}/'].attrs['timeRecordInterval'])
                            start_time = hdf_file[f'/WaterLevel/{i}/{group}/'].attrs['startDateTime']
                            start_time = dateutil.parser.parse(start_time)
                            end_time = start_time + \
                                datetime.timedelta(
                                    seconds=((number_of_times*interval)-interval))
                            timestamps = pd.date_range(
                                start_time, end_time, freq=f'{interval}S')

                            group_df = pd.DataFrame(
                                stn_data, index=timestamps, columns=[stn_label])
                            if data_df.empty:
                                data_df = group_df
                            else:
                                data_df = pd.concat(
                                    [data_df, group_df], axis=1)
        ...
        ### S111 Conversion ###
            ...
            for group in hdf_file['/SurfaceCurrent/SurfaceCurrent.01/']:
                if group != 'Positioning':
                    ...
                    stn_id = hdf_file[
                        f'/SurfaceCurrent/SurfaceCurrent.01/{group}/'].attrs['stationIdentification']
                    ...
                    # Generate Timestamps for index
                    number_of_times = int(
                        hdf_file[f'/SurfaceCurrent/SurfaceCurrent.01/{group}/'].attrs['numberOfTimes'])
                    interval = int(
                        hdf_file[f'/SurfaceCurrent/SurfaceCurrent.01/{group}/'].attrs['timeRecordInterval'])
                    start_time = hdf_file[
                        f'/SurfaceCurrent/SurfaceCurrent.01/{group}/'].attrs['startDateTime']
                    start_time = dateutil.parser.parse(start_time)
                    end_time = start_time + \
                        datetime.timedelta(
                            seconds=((number_of_times*interval)-interval))
                    ...
                    group_df = pd.DataFrame(
                        stn_speed, index=timestamps, columns=[f'{stn_label}_wcs'])
                    dir_df = pd.DataFrame(stn_direction, index=timestamps, columns=[
                                          f'{stn_label}_wcd'])
