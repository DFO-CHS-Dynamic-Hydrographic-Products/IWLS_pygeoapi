To install the IWLS PygeoAPI process plugin development server:
    ...
    4. Set environnment variables and generate open api config file:
        export PYGEOAPI_CONFIG=config_dev.yml
        export PYGEOAPI_OPENAPI=config.yml
        pygeoapi openapi generate $PYGEOAPI_CONFIG > $PYGEOAPI_OPENAPI 
    5. pygeoapi serve to start server
...
Sample syntax for S-104 process query:
    ...
    - Unsuccessful processes will return json containing error message instead of a zip file

On WSL, environment variables must be set every session